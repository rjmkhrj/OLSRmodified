...
RoutingProtocol::MprComputation ()
{
 ...
 NeighborSet N;
  ...
  for (NeighborSet::const_iterator nai = N.begin(); nai != N.end(); )            //New Protocol
  {
     if (nai->willingness == OLSR_WILL_NEVER)
     {
        //NS_LOG_INFO ("Hello msg");
        nai = N.erase (nai);
     }
     else
        nai++;
      ...
      if (onlyOne)
        {
          ...
          for (NeighborSet::const_iterator neighbor = N.begin(); neighbor != N.end(); )      //New Protocol (Removing the nodes in N which have been already put in the MPR set
          {
             if (neighbor->neighborMainAddr == twoHopNeigh->neighborMainAddr)
             {
                neighbor = N.erase(neighbor);
             }
             else
                 neighbor++;
          }
...
for (TwoHopNeighborSet::iterator a = N2.begin(); a != N2.end(); a++)
{
   bool x = false;
  for (NeighborSet::iterator b = N.begin(); b != N.end(); b++)
  {
     if (a->neighborMainAddr == b->neighborMainAddr)
       x = true;
  }
...
//NS_LOG_INFO ("Size of N start " <<N.size());
//NS_LOG_INFO ("Size before " <<N2.size());


//Uptill this point only the 2hop nodes which are connected by only a single one hop neighbor has been considered. 
/*for (NeighborSet::iterator px = N.begin(); px != N.end(); px++)
NS_LOG_INFO ("one hop addr" << px->neighborMainAddr);*/

while(!N2.empty()) 
{
   std::set<Ipv4Address> coveredNeighbors;
   NeighborTuple const* min = NULL;
   Ipv4Address ss;
   for (NeighborSet::const_iterator neighbor = N.begin(); neighbor != N.end(); neighbor++)    //New Protocol
   {
       const NeighborTuple &temp = *neighbor;
       if ( min==NULL || ((int)temp.MPRselectset < (int)min->MPRselectset))
       {
          min = &temp;
       }
       else
       {
          if ((int)temp.MPRselectset == (int)min->MPRselectset)
          {
              if (Degree (*min) < Degree (temp))
              {
                 min = &temp;
              }
          }
  ...
  if(min != NULL)
  {
      mprSet.insert (min->neighborMainAddr);
     //NS_LOG_INFO ("Size second" << N2.size());
     for (TwoHopNeighborSet::iterator twoHopNeigh = N2.begin ();
       twoHopNeigh != N2.end (); twoHopNeigh++)
     {

        if (twoHopNeigh->neighborMainAddr == min->neighborMainAddr)
        {
          //NS_LOG_INFO ("Insert");
          coveredNeighbors.insert (twoHopNeigh->neighborMainAddr);
          coveredNeighbors.insert (twoHopNeigh->twoHopNeighborAddr);
        }
     }

     for (TwoHopNeighborSet::iterator twoHop = N2.begin ();
       twoHop != N2.end (); )
     {
       if ((coveredNeighbors.find (twoHop->neighborMainAddr) != coveredNeighbors.end()) || (coveredNeighbors.find (twoHop->twoHopNeighborAddr) != coveredNeighbors.end()))
       {
          //NS_LOG_INFO ("Delete");
         twoHop = N2.erase(twoHop);
       }
       else
         twoHop++;
     }   

  ss = min->neighborMainAddr;
  //NS_LOG_INFO ("min is " << min->neighborMainAddr);   
  } 

     int count = 0;
     for (NeighborSet::const_iterator nn = N.begin(); nn != N.end(); )
     {
        if ((nn->neighborMainAddr) == ss)
        {
            nn = N.erase (nn);
            count++;
        }
...
/*for (TwoHopNeighborSet::iterator a = N2.begin(); a != N2.end(); a++)
{
  NS_LOG_INFO (a->neighborMainAddr); 
}*/
...
#endif  //NS3_LOG_ENABLE*/
  NS_LOG_INFO ("Size of MPR " << mprSet.size());
...
RoutingProtocol::SendHello ()
{
  ...
  //NS_LOG_INFO ("Count is" << count);
  msg.SetMPRselectsize (count);                    //New Protocol
  //NS_LOG_INFO ("Here over " << x.size());
  ...
  if (nb_tuple != NULL)
    {
     ...
     // NS_LOG_INFO ("info " << (int)nb_tuple->MPRselectset);
